<?php
/**
* @version $Id: mod_vlogin.php,v 1.0 2005/12/04 21:03:41 mmbrich Exp $
* @package Mambo
* @copyright (C) 2005 - 2010 Foss Labs
...
require_once('modules/vt_classes/VTigerUser.class.php');
$VTUser = new VTigerUser();
...
if ( $my->id ) {
	...
	$database->setQuery( $query );
	$password = $database->loadResult();
	$VTUser->username = $my->username;
	$VTUser->password = $password;
	$result = $VTUser->authUser();
	if($result != "false") {
		session_start();
		$_SESSION["authenticated_user_id"] = $result;
	}
	// END VTIGER
	?>
